
import React from "react"
 import './SocailM.css'

const SocailMedia = (props)=>{
    return(

<div className='social-media'>
 
  

 <a href='https://www.linkedin.com/in/aamer-feroz/' target='_blank' rel='noreferrer'> <i className='fab fa-linkedin'></i></a>
 <a href='https://github.com/alamirshaikh' target='_blank' rel='noreferrer'> <i className='fab fa-github'></i></a>
 <a href='https://twitter.com/aamerferoz' target='_blank' rel='noreferrer'> <i className='fab fa-twitter'></i></a>
 <a href='https://www.instagram.com/aamer.feroz/' target='_blank' rel='noreferrer'> <i className='fab fa-instagram'></i></a>
</div>
    );
}

export default SocailMedia;